...
extern void Filter(unsigned char * in, unsigned char * out, int width, int height) ;
extern unsigned long long int my_timer() ;
		...
		if ( SDL_PollEvent(&event) ) {
			switch (event.type) {
				...
				switch (event.key.keysym.sym) {
					...
					printf("Start filtering...  ");
					t1 = my_timer();
					Filter_c(image->pixels, tmpBuf, image->w, image->h) ;
					t2 = my_timer();
					printf("Done.\n pomiar: %llu\n", t2-t1);
					...
					printf("Start filtering...  ");
					t1 = my_timer();
					Filter_c2(image->pixels, tmpBuf, image->w, image->h) ;
					t2 = my_timer();
					printf("Done.\n pomiar: %llu\n", t2-t1);
